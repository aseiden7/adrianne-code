...
L_range         <- c(25, 83)  # min/max HCL luminance used across ALL plants
...
build_region_key <- function(regions, labels, l_range = L_range) {
  n <- length(regions)
  l_sorted <- seq(l_range[1], l_range[2], length.out = n)
  ...
  key_df$text_color <- ifelse(key_df$L > 55, "black", "white")
  ...
  ggplot(key_df, aes(x = 1, y = region, fill = fill)) +
    geom_tile(color = "black", width = 1, height = 0.95) +
...
preview_region_palette <- function(regions, methods = names(method_hues)) {
  ...
  for (i in seq_len(nrow(grid))) {
    ...
    rect(ri - 1, n_m - mi, ri, n_m - mi + 1, col = grid$hex[i], border = "white")
  }
...
method_levels <- c("raw", "h2o20", "h2o30")  # EDIT: full canonical method list, in order
