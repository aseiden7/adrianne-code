...
# Calculate Alkyl/O-Alkyl Ratios

Calculate the alkyl/O-alkyl ratio for each sample and save updated CSV with ratios. Then calculate mean ratios grouped by crop type and timepoint.
...
knitr::kable(ratio_means, 
             caption = "Mean Alkyl/O-Alkyl Ratios by Crop and Timepoint") %>%
  kableExtra::kable_styling()

# Save the summary table
# Save the summary table
write.csv(ratio_means, file.path(outputs_folder, "Alkyl_O-Alkyl_ratio_means.csv"), row.names = FALSE)
```

# Plot Alkyl/O-Alkyl Ratios Over Time
...
# Create the plot
p <- ggplot(plot_data, aes(x = timepoint, y = Alkyl_O.Alkyl_Ratio, fill = crop_time)) +
...
Create plots showing mean NMR spectra for each crop-timepoint combination with standard deviation and full range.

```{r plot-averaged-spectra, message=FALSE, warning=FALSE, fig.width=12, fig.height=10}
library(ggplot2)
library(dplyr)
library(tidyr)
...
color_map <- c(
  "soy_wk0" = crop_timepoint_colors$soy["wk0"],
  "soy_wk10" = crop_timepoint_colors$soy["wk10"],
  "soy_wk40" = crop_timepoint_colors$soy["wk40"],
  "rice_wk0" = crop_timepoint_colors$rice["wk0"],
  "rice_wk10" = crop_timepoint_colors$rice["wk10"],
  "rice_wk40" = crop_timepoint_colors$rice["wk40"],
  "wheat_wk0" = crop_timepoint_colors$wheat["wk0"],
  "wheat_wk10" = crop_timepoint_colors$wheat["wk10"],
  "wheat_wk40" = crop_timepoint_colors$wheat["wk40"]
)

# Create the plot
p_spectra <- ggplot(spectra_stats, aes(x = ppm, y = mean_intensity, color = crop_time, fill = crop_time)) +
  # Full range (most transparent)
  geom_ribbon(aes(ymin = min_intensity, ymax = max_intensity), alpha = 0.22, color = NA) +
  # +/- 1 SD (medium transparency)
  geom_ribbon(aes(ymin = lower_sd, ymax = upper_sd), alpha = 0.55, color = NA) +
  # Mean line (solid)
  geom_line(linewidth = 0.8) +
  facet_wrap(~ crop_time, ncol = 3, scales = "free_y") +
  scale_x_reverse() +
...
ggsave(filename = file.path(outputs_folder, "NMR_spectra_averaged_by_crop_timepoint.png"),
       plot = p_spectra, width = 12, height = 10, dpi = 300)
```
