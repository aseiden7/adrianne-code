...
nmr_img <- readPNG("NMR-annotations.png")
img_dim <- dim(nmr_img)
...
# Create annotation bars for each crop (to match facet columns)
bonanomi_regions_faceted_indiv <- bind_rows(
  bonanomi_regions %>% mutate(crop = "wheat"),
  bonanomi_regions %>% mutate(crop = "rice"),
  bonanomi_regions %>% mutate(crop = "soy")
)

p_annotation_indiv <- ggplot(bonanomi_regions_faceted_indiv) +
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = 0, ymax = 0.7, fill = color)) +
  scale_fill_identity() +
  scale_x_reverse(limits = c(230, -10)) +
  scale_y_continuous(limits = c(0, 0.7), expand = c(0, 0)) +
...
# Combine plots vertically
p_combined_indiv <- p_individual / p_annotation_indiv + plot_layout(heights = c(20, 1))

# Display the plot
print(p_combined_indiv)

# Save the plot
ggsave_themed(filename = "NMR_spectra_individual_overlaid_by_timepoint.png",
              plot = p_combined_indiv, width = 12, height = 5, dpi = 300)
```
