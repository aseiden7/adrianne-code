...
p <- ggplot(plot_data, aes(x = timepoint, y = Alkyl_O.Alkyl_Ratio, fill = crop, color = crop)) +
  geom_point(aes(group = interaction(timepoint, crop)), 
             position = position_jitterdodge(jitter.width = 0.02, dodge.width = 0.6), 
             alpha = 0.9, size = 4.5) +
...
```{r plot-averaged-spectra, message=FALSE, warning=FALSE, fig.width=12, fig.height=8}
library(ggplot2)
library(dplyr)
library(tidyr)
...
p_spectra <- ggplot(spectra_stats, aes(x = ppm, y = mean_intensity, color = crop_time, fill = crop_time)) +
  ...
  geom_ribbon(aes(ymin = min_intensity, ymax = max_intensity), alpha = 0.17, color = NA) +
...
ggsave(filename = file.path(outputs_folder, "NMR_spectra_averaged_by_crop_timepoint.png"),
       plot = p_spectra, width = 12, height = 8, dpi = 300)
```
...
```{r plot-alkyl-zoom, message=FALSE, warning=FALSE, fig.width=12, fig.height=8}
...
# Save the plot
ggsave(filename = file.path(outputs_folder, "NMR_spectra_alkyl_region_27-36ppm.png"),
       plot = p_alkyl, width = 12, height = 8, dpi = 300)
...
p_aSub <- ggplot(spectra_stats_aSub, aes(x = ppm, y = mean_intensity, color = crop_time, fill = crop_time)) +
  ...
  labs(
    x = "Chemical Shift (ppm)",
    y = "Intensity",
    title = "Alkyl Suberin Region (27-36 ppm)"
  ) +
  theme_minimal() +
  theme(
    legend.position = "none",
...
# Save the plot
ggsave(filename = file.path(outputs_folder, "NMR_spectra_aSub_27-36ppm.png"),
       plot = p_aSub, width = 12, height = 8, dpi = 300)
```
