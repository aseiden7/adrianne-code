...
for (crop_name in c("wheat", "rice", "soy")) {
  crop_data <- ratio_stats %>%
    filter(crop == crop_name)
  
  if (nrow(crop_data) > 1) {
    model <- lm(Mean_simple_microbial ~ timepoint_numeric, data = crop_data)
...
for (crop_name in c("wheat", "rice", "soy")) {
  ...
  if (nrow(crop_data) > 1) {
    model <- lm(Mean_total_microbial ~ timepoint_numeric, data = crop_data)
    summary_model <- summary(model)
...
for (crop_name in c("wheat", "rice", "soy")) {
  ...
  if (nrow(crop_data) > 1) {
    model <- lm(Mean_simple_complex ~ timepoint_numeric, data = crop_data)
    summary_model <- summary(model)
