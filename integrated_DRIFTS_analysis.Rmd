...
# Print sample counts for week 0
wk0_counts <- root0_data %>%
  select(filename, crop) %>%
  distinct() %>%
  count(crop) %>%
  arrange(match(crop, c("noPlant", "wheat", "rice", "soy"))) %>%
  mutate(`Week 0 samples` = paste(n, crop, sep = "&ensp;&nbsp;")) %>%
  select(`Week 0 samples`)

knitr::kable(wk0_counts, format = "markdown")
...
# Print sample counts for week 10
wk10_counts <- root10_data %>%
  select(filename, crop) %>%
  distinct() %>%
  count(crop) %>%
  arrange(match(crop, c("noPlant", "wheat", "rice", "soy"))) %>%
  mutate(`Week 10 samples` = paste(n, crop, sep = "&ensp;&nbsp;")) %>%
  select(`Week 10 samples`)

knitr::kable(wk10_counts, format = "markdown")
...
# Print sample counts for week 40
wk40_counts <- root40_data %>%
  select(filename, crop) %>%
  distinct() %>%
  count(crop) %>%
  arrange(match(crop, c("noPlant", "wheat", "rice", "soy"))) %>%
  mutate(`Week 40 samples` = paste(n, crop, sep = "&ensp;&nbsp;")) %>%
  select(`Week 40 samples`)

knitr::kable(wk40_counts, format = "markdown")
...
# Add complex plant region annotation
  ...
  annotate("text", x = 1200, y = max(wheat$absorbance), label = "Complex Plant", 
           size = 3, hjust = 0.5)
wheat_plot
# Print unique ID and timepoint combinations for wheat
wheat_combinations <- wheat %>%
  select(ID, timepoint) %>%
  distinct() %>%
  arrange(ID, timepoint) %>%
  group_by(timepoint) %>%
  summarise(pot_ids = paste("pot", ID, collapse = ",&ensp;", sep = ""), .groups = 'drop') %>%
  pivot_wider(names_from = timepoint, values_from = pot_ids)

knitr::kable(wheat_combinations, format = "markdown")
...
# Add complex plant region annotation
  ...
  annotate("text", x = 1200, y = max(rice$absorbance), label = "Complex Plant", 
           size = 3, hjust = 0.5)
rice_plot
# Print unique ID and timepoint combinations for rice
rice_combinations <- rice %>%
  select(ID, timepoint) %>%
  distinct() %>%
  arrange(ID, timepoint) %>%
  group_by(timepoint) %>%
...
knitr::kable(rice_combinations, format = "markdown")
...
soy_plot
# Print unique ID and timepoint combinations for soy
soy_combinations <- soy %>%
  select(ID, timepoint) %>%
  distinct() %>%
  arrange(ID, timepoint) %>%
  group_by(timepoint) %>%
...
knitr::kable(soy_combinations, format = "markdown")
```
