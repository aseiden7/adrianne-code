...
# Set default theme for all plots (toggle between theme_light_custom() and theme_dark_custom())
theme_set(theme_dark_custom())
theme_suffix <- "-dark"  # Change to "-light" when using theme_light_custom()
annotation_text_color <- ifelse(theme_suffix == "-dark", "white", "black")  # Change to "black" when using theme_light_custom()
annotation_fill_color <- ifelse(theme_suffix == "-dark", "black", "white")  # Set annotation fill color based on theme
# Wrapper function for ggsave with automatic theme suffix
...
crop_colors <- c('wheat' = '#7B41A9', 'rice' = '#FC9D33', 'soy' = '#FE318E', 'noPlant' = '#98C65D')
...
### Line plots
```{r line-crop-plots, results='asis'}
...
crop_timepoint_colors_all <- list(
  ...
  noPlant = c("wk30" = "#58792A", "wk40" = "#374B1B")
)
...
create_ratio_scatterplot <- function(data, y_metric, y_metric_se, title, y_label,
  ...
  plot <- ggplot(data, aes(x = timepoint_numeric, y = !!sym(y_metric), color = crop)) +
    ...
    geom_errorbar(aes(ymin = !!sym(y_metric) - !!sym(y_metric_se),
                  ...
                  width = 1.5, alpha = 0.6, linewidth = 0.8) +
...
### Line plots
```{r line-time-suberin-plots, results='asis'}
# Generic function to create line plots for any timepoint
create_timepoint_line_plot <- function(data, timepoint_week, title = NULL) {
  # Filter data for specific timepoint
  filtered_data <- data %>%
