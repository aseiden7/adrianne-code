...
create_suberin_ridge_plot <- function(data, timepoint_week, title = NULL) {
  ...
  ridge_plot <- filtered_data %>%
    ...
    geom_vline(xintercept = 1660, linetype = "dashed", linewidth = 0.23, color = "#4d653a") +
...
# Suberin peak annotations
    ...
    geom_vline(xintercept = 1506, linetype = "dotted", linewidth = 0.3, color = "#5f3e38") +
   # spacer
    annotate("text", x = 1900, y = 10.1, label = " ", 
             size = 3, hjust = 0.5) +
    # Add aliphatic annotation
    annotate("text", x = 2600, y = text_y, label = "aliphatic suberin", 
             size = 3, hjust = 0.5) +
    # Add esters annotation
    annotate("text", x = 1910, y = text_y, label = "suberin esters", 
             size = 3, hjust = 0.5) +
    # Add aromatic annotation
    annotate("text", x = 1300, y = text_y, label = "aromatic suberin", 
             size = 3, hjust = 0.5)
...
create_crop_line_plot <- function(data, crop_name, color_palette) {
 # Filter data for specific crop
  ...
  line_plot <- crop_data %>%
    ...
    # Suberin esters (1737-1740 cm-1)
    annotate("rect", xmin = 1733, xmax = 1742, ymin = -Inf, ymax = Inf, 
             alpha = 0.15, fill = "#4d653a") +
    geom_vline(xintercept = 1737, linetype = "dashed", linewidth = 0.23, color = "#4d653a") +
    geom_vline(xintercept = 1660, linetype = "dashed", linewidth = 0.23, color = "#4d653a") +
    ...
    geom_vline(xintercept = 1506, linetype = "dotted", linewidth = 0.3, color = "#5f3e38") +
    # Add aliphatic annotation
    annotate("text", x = 2660, y = max(crop_data$normalized_absorbance)-0.05, label = "aliphatic suberin", 
             size = 3, hjust = 0.5, color = "#163a6c") +
    # Add esters annotation
    annotate("text", x = 1910, y = max(crop_data$normalized_absorbance), label = "suberin esters", 
             size = 3, hjust = 0.5, color = "#2b3c1d") +
    # Add aromatic annotation
    annotate("text", x = 1300, y = max(crop_data$normalized_absorbance)+0.02, label = "aromatic suberin", 
             size = 3, hjust = 0.5, color = "#3f2622")
  return(list(plot = line_plot, data = crop_data))
}
