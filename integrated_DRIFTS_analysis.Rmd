...
create_ratio_scatterplot <- function(data, y_metric, y_metric_se, title, y_label,
                                     regression_stats, crop_colors) {
  ...
  for (i in seq_len(nrow(regression_stats))) {
    ...
    # x position: 85% of the way across
    label_x <- min_x + (max_x - min_x) * 0.85
    # y position: staggered vertically near the top (adjust based on crop order)
    # wheat: highest, rice: middle, soy: lowest
    crop_order <- match(crop_name, c("wheat", "rice", "soy"))
    label_y <- max_y - (crop_order - 1) * (y_range * 0.09)
    ...
    # Add text annotations in crop color
    plot <- plot +
      annotate("text",
               x = label_x, y = label_y,
               label = label_text,
...
simple_micr_plot <- create_ratio_scatterplot(
  ...
  title = "Simple Plant : Microbial Ratio Over Time",
...
ggsave_themed("simple_plant_microbial_ratio_scatterplot.png", simple_micr_plot, width = 6, height = 4)
...
total_micr_plot <- create_ratio_scatterplot(
  ...
  title = "Total Plant (Simple + Complex) : Microbial Ratio Over Time",
...
ggsave_themed("total_plant_microbial_ratio_scatterplot.png", total_micr_plot, width = 6, height = 4)
...
simple_complex_plot <- create_ratio_scatterplot(
  ...
  title = "Simple Plant : Complex Plant Ratio Over Time",
...
ggsave_themed("simple_complex_ratio_scatterplot.png", simple_complex_plot, width = 6, height = 4)
```
