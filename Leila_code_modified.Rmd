...
# setwd("C:/Users/adria/Box/Salk Institute Project/AKS Salk files/Adrianne_FTIRdata") # nolint: line_length_linter.

# This first section of the code is loading data from the DPT files

# Set data folder (don't need to change working directory)
# # Windows path
# dpt_folder <- "C:/Users/adria/Box/Salk Institute Project/AKS Salk files/Adrianne_FTIRdata/DPT_files" # nolint: line_length_linter.
# # relative path
# dpt_folder <- "../Adrianne_FTIRdata/DPT_files"

# # path from local repository
# box_base <- Sys.getenv("BOX_BASE")
# if (box_base == "") stop("BOX_BASE environment variable is not set!")
...
# # folder path from local repository
# box_base <- Sys.getenv("BOX_BASE")
# if (box_base == "") stop("BOX_BASE environment variable is not set!")
...
# Save as CSV
today <- Sys.Date()
meta_wide_table_csv <- paste0("FTIR_meta_wide_", today, ".csv")
...
# Save as CSV
today <- Sys.Date()
meta_long_table_csv <- paste0("FTIR_meta_long_", today, ".csv")
...
Also, I export a csv file in every chunk, for troubleshooting purposes. This is not necessary for the final code.

## Importing the list of file names
Felt cute, might delete later.
```{r import-filenames, echo=TRUE, message=FALSE, warning=FALSE}
# Need to have all of the datafiles you want to import in the same directory.
# # Windows path
# data1 <- list.files(
#   path = "C:/Users/adria/Box/Salk Institute Project/AKS Salk files/Adrianne_FTIRdata/DPT_files", # nolint: line_length_linter.
#   full.names = TRUE
# )
# # relative path
# data1 <- list.files(
#   path = "../Adrianne_FTIRdata/DPT_files",
#   full.names = TRUE
# )
...
# if (box_base == "") stop("BOX_BASE environment variable is not set!")
# data1 <- list.files(
#   path = file.path(box_base, "Salk Institute Project/AKS Salk files/Adrianne_FTIRdata/DPT_files",
#   full.names = TRUE
# )
...
if (total_rows %% n_samples == 0) {
  data2$sample <- sort(rep(1:n_samples, measurements_per_sample))
} else {
...
comb_name <- paste0("spectra_combined_test_", today, ".csv")
...
# First, define the sample variable
sample <- unique(comb$sample)

# Then create the data frame with the correct number of rows
ints <- as.data.frame(matrix(NA, nrow = length(sample), ncol = 11))

# Now assign the sample column
ints$sample <- sample

# Check dimensions match before assigning metadata
print(paste("ints rows:", nrow(ints)))
print(paste("meta_table rows:", nrow(meta_table)))
...
names(ints)[8:9] <- c("int_2839_2870", "int_2898_2976")
names(ints)[10:11] <- c("int_1580_1660", "int_1500_1550")
head(ints)

##export to check - note there are no absorbance values yet
today <- format(Sys.Date(), "%Y-%m-%d")
comb_names <- paste0("spectra_combined_names_test_", today, ".csv")
...
head(ints)
today <- format(Sys.Date(), "%Y-%m-%d")
ints_name <- paste0("integrated_peak_data_", today, ".csv")
...
Still troubleshooting this code.
...
ftir_data$total_peak_area <- ftir_data$aliphatic + ftir_data$int_1580_1660 + ftir_data$int_1500_1550
ftir_data$simple_plant_prop <- ftir_data$aliphatic / ftir_data$total_peak_area
ftir_data$complex_plant_prop <- ftir_data$int_1500_1550 / ftir_data$total_peak_area
ftir_data$microbial_plant_prop <- ftir_data$int_1580_1660 / ftir_data$total_peak_area
ftir_data$simple_microbial_prop <- ftir_data$simple_plant_prop / ftir_data$microbial_plant_prop
ftir_data$complex_microbial_prop <- ftir_data$complex_plant_prop / ftir_data$microbial_plant_prop

# Save processed data
today <- format(Sys.Date(), "%Y-%m-%d")
proc_name <- paste0("processed_int_data_", today, ".csv")
...
wk0_wheat_root_data <- ftir_data[ftir_data$crop == "wheat" & ftir_data$timepoint == "wk0", ]
wk10_wheat_root_data <- ftir_data[ftir_data$crop == "wheat" & ftir_data$timepoint == "wk10", ]
wk0_rice_root_data <- ftir_data[ftir_data$crop == "rice" & ftir_data$timepoint == "wk0", ]
wk10_rice_root_data <- ftir_data[ftir_data$crop == "rice" & ftir_data$timepoint == "wk10", ]
wk0_soy_root_data <- ftir_data[ftir_data$crop == "soy" & ftir_data$timepoint == "wk0", ]
wk10_soy_root_data <- ftir_data[ftir_data$crop == "soy" & ftir_data$timepoint == "wk10", ]
...
calculate_group_stats <- function(data, group_vars) {
  data %>%
    ...
    summarise(
      ...
      Mean_1580_1660 = mean(int_1580_1660, na.rm = TRUE),
      SE_1580_1660 = sd(int_1580_1660, na.rm = TRUE) / sqrt(n()),
      Mean_1500_1550 = mean(int_1500_1550, na.rm = TRUE),
      SE_1500_1550 = sd(int_1500_1550, na.rm = TRUE) / sqrt(n()),
      ...
      Mean_microbial_prop = mean(microbial_plant_prop, na.rm = TRUE),
      SE_microbial_prop = sd(microbial_plant_prop, na.rm = TRUE) / sqrt(n()),
...
write.csv(crop_timepoint_stats, "FTIR_Stats_Crop_Timepoint.csv", row.names = FALSE)
write.csv(crop_stats, "FTIR_Stats_Crop.csv", row.names = FALSE)
write.csv(timepoint_stats, "FTIR_Stats_Timepoint.csv", row.names = FALSE)
```
...
print(crop_combined)
ggsave("FTIR_by_Crop.pdf", plot = crop_combined, width = 18, height = 9, units = "in", dpi = 300)
...
ggsave("FTIR_by_Timepoint.pdf", plot = timepoint_combined, width = 18, height = 9, units = "in", dpi = 300)
...
interaction_plot <- ggplot(crop_timepoint_stats) +
  ...
  theme_classic() +
  scale_fill_manual("Timepoint", values = c("#440154", "#31688e")) +
...
ggsave("FTIR_Crop_Timepoint_Interaction.pdf", plot = interaction_plot, 
...
p_crop_stacked <- ggplot(crop_stacked_data, aes(x = crop, y = proportion * 100, fill = func_type)) +
  ...
  scale_fill_manual(values = c("#9E9AC8", "#DADAEB", "#6A51A3"),
...
p_timepoint_stacked <- ggplot(timepoint_stacked_data, aes(x = timepoint, y = proportion * 100, fill = func_type)) +
  geom_col(position = "stack") +
  scale_fill_manual(values = c("#FA9FB5", "#FDE0DD", "#F768A1"),
                    breaks = c("Microbial", "Simple Plant", "Complex Plant")) +
  theme_classic() +
  labs(x = "Timepoint",
       y = "Functional Group Amount (%)",
