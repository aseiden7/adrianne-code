...
corr_spectra <- file.path(box_base, "Salk Institute Project/AKS Salk files/Camryn_DRIFTSdata/corrected_data")
# set dpt_folder path
...
```
# Overview
Use this markdown for quality control of OPUS binary files. It reads the files, extracts metadata (i.e. method parameters) and plots the spectra. It also stacks spectra for the same sample number, crop, and timepoint to check for consistency. Problematic spectra can be identified and then filtered out in DRIFTS_Camryn.Rmd.
...
# # Mac path
# corr_spectra <- "/Users/adrianneseiden/Library/CloudStorage/Box-Box/Salk Institute Project/AKS Salk files/Camryn_DRIFTSdata/corrected_data" # nolint
#  # Windows path
# corr_spectra <- "C:/Users/adria/Box/Box-Box/Salk Institute Project/AKS Salk files/Camryn_DRIFTSdata/corrected_data" # nolint
# # Relative path
# corr_spectra <- "../Camryn_DRIFTSdata/corrected_data"
...
extract_sample_number <- function(filename) {
  m <- regexpr("[A-Za-z]{3}[0-9]+", filename)
  if (m[1] != -1) {
...
extract_sample_type <- function(filename) {
  m <- regexpr("rhizoSoil|bulkSoil", filename, ignore.case = TRUE)
  if (m[1] != -1) {
