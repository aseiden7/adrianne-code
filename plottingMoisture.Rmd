...
def plot_sheet_data(df, sheet_name):
    ...
    plt.savefig(f'{today_date}_soilMoisture_{sheet_name}.png', dpi=300, bbox_inches='tight')
...
def load_and_analyze_data(file_path=file_path):
    ...
    today_date = datetime.now().strftime('%y%m%d')
    kpa_stats.to_csv(f'{today_date}_kpa_stats.csv', index=True)
    
    return combined_df, kpa_stats

# Load the data and get statistics
file_path = 'Compiled.moisture.xlsx'
...
def load_and_analyze_recent_data(file_path=file_path, weeks=10):
    ...
    today_date = datetime.now().strftime('%y%m%d')
    stats_file = f'{today_date}_kpa_stats.csv'
    ...
    if os.path.exists(stats_file):
        ...
        with pd.ExcelWriter(f'{today_date}_kpa_stats.xlsx') as writer:
...
def plot_recent_data(df, sheet_name, weeks=10):
    ...
    # Save the figure with today's date in YYMMDD format
    today_date = datetime.now().strftime('%y%m%d')
    plt.savefig(f'{today_date}_soilMoisture_{sheet_name}_recent{weeks}w.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_sheet_data_combined(df, timestamp_col):
    ...
    # Save the figure with today's date in YYMMDD format
    today_date = datetime.now().strftime('%y%m%d')
    plt.savefig(f'{today_date}_soilMoisture_combined.png', dpi=300, bbox_inches='tight')
