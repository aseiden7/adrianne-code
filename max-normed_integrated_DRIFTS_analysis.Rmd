...
ftir_data <- ftir_data %>%
  mutate(
    ...
    complex_simple_prop = complex_plant_prop / simple_plant_prop,
    complex_mic_tot = complex_plant_prop + microbial_prop,
    simple_v_all_complex = simple_plant_prop / complex_mic_tot
  )
...
ratio_stats <- ftir_data %>%
  ...
  summarise(
    ...
    Mean_complex_simple = mean(complex_simple_prop, na.rm = TRUE),
    SE_complex_simple = sd(complex_simple_prop, na.rm = TRUE) / sqrt(n()),
    Mean_simple_v_all_complex = mean(simple_v_all_complex, na.rm = TRUE),
    SE_simple_v_all_complex = sd(simple_v_all_complex, na.rm = TRUE)/sqrt(n()),
    .groups = 'drop'
  )
...
# Calculate regression statistics for each crop (complex plant : simple plant ratio)
# Using all individual data points, not just the 3 averaged points
complex_simple_regression_stats <- data.frame()
for (crop_name in c("wheat", "rice", "soy")) {
  crop_data <- ftir_data %>%
    filter(crop == crop_name) %>%
    mutate(timepoint_numeric = as.numeric(str_extract(timepoint, "\\d+")))
  ...
  if (nrow(crop_data) > 1) {
    model <- lm(complex_simple_prop ~ timepoint_numeric, data = crop_data)
    summary_model <- summary(model)
    ...
    complex_simple_regression_stats <- rbind(complex_simple_regression_stats,
      data.frame(
        crop = crop_name,
        r_squared = r_squared,
        p_value = p_value,
...
# Calculate regression statistics for each crop (simple plant : complex + mic. ratio)
# Using all individual data points, not just the 3 averaged points
simple_v_all_complex_regression_stats <- data.frame()
for (crop_name in c("wheat", "rice", "soy")) {
  crop_data <- ftir_data %>%
    filter(crop == crop_name) %>%
    mutate(timepoint_numeric = as.numeric(str_extract(timepoint, "\\d+")))
  ...
  if (nrow(crop_data) > 1) {
    model <- lm(simple_v_all_complex ~ timepoint_numeric, data = crop_data)
    summary_model <- summary(model)
    ...
    simple_v_all_complex_regression_stats <- rbind(simple_v_all_complex_regression_stats,
      data.frame(
        crop = crop_name,
        r_squared = r_squared,
        p_value = p_value,
...
# Create Complex Plant : Simple Plant Ratio scatterplot
complex_simple_plot <- create_ratio_scatterplot(
  data = ratio_stats,
  y_metric = "Mean_complex_simple",
  y_metric_se = "SE_complex_simple",
  title = "",
  y_label = "Complex Plant : Simple Plant Ratio",
  regression_stats = complex_simple_regression_stats,
  crop_colors = crop_colors
)

# Create Simple Plant : Complex + microbial Ratio scatterplot
simple_cm_plot <- create_ratio_scatterplot(
  data = ratio_stats,
  y_metric = "Mean_simple_v_all_complex",
  y_metric_se = "SE_simple_v_all_complex",
  title = "",
  y_label = "Simple Plant : Complex + Microbial Ratio",
  regression_stats = simple_v_all_complex_regression_stats,
  crop_colors = crop_colors
)

print(simple_complex_plot)
ggsave_themed("simple_complex_ratio_scatterplot.png", simple_complex_plot)
print(simple_cm_plot)
ggsave_themed("simple_complex+microbial_ratio_scatterplot.png", simple_cm_plot)
```
  ...
  ridge_plot <- filtered_data %>%
   ...
   # spacer
    ...
    annotate("label", x = 1310, y = max_y + 0.1, label = "suberin fingerprint", 
             size = 3.3, hjust = 0.5, fill = annotation_text_color, label.size = NA, color = annotation_fill_color)
