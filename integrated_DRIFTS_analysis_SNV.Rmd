...
# Overview
This document imports and analyzes FTIR DRIFTS data from DPT files, normalizes \& plots the spectra, calculates functional group proportions, and visualizes the results. The general analysis pipeline is based on methods from Mainka et al. (2022), with modifications to fit the Salk project variables (crop type and decomposition time). 
...
## save as csv for record-keeping
write.csv(dpt_data_roots, file.path(outputs_folder, "dpt_data_normalized_roots.csv"), row.names = FALSE)
```
...
perform_spectral_integration <- function(dpt_data) {
  ...
  ftir_data$total_plant_prop <- ftir_data$simple_plant_prop + ftir_data$complex_plant_prop
  ftir_data$total_plant_microbial_prop <- ftir_data$total_plant_prop / ftir_data$microbial_prop
  ftir_data$simple_complex_prop <- ftir_data$simple_plant_prop / ftir_data$complex_plant_prop
...
create_suberin_ridge_plot <- function(data, timepoint_week, title = NULL) {
  ...
  ridge_plot <- filtered_data %>%
   ...
   # spacer
    ...
    annotate("text", x = 2626, y = max_y*1.19, label = "aliphatic suberin", 
             size = 3.3, hjust = 0.5, color = annotation_text_color) +
    # Add esters annotation
    annotate("text", x = 1930, y = max_y*1.32, label = "suberin esters", 
             size = 3.3, hjust = 0.5, color = annotation_text_color) +
    # Add fingerprint annotation
    annotate("label", x = 1310, y = max_y*1.4, label = "suberin fingerprint", 
  ...
  crop_plot <- ggplot(crop_data,
   ...
   # Suberin peak annotations
    ...
    annotate("text", x = 1919, y = max(crop_data$normalized_absorbance), label = "suberin esters", 
             size = 3.3, hjust = 0.5, color = annotation_text_color) +
    # Add fingerprint annotation
    annotate("label", x = 1290, y = max(crop_data$normalized_absorbance)+0.04, label = "suberin fingerprint", 
             size = 3.3, hjust = 0.5, color = annotation_text_color)
...
base_plot_three <- ggplot() +
  ...
  scale_x_reverse(
    ...
    # Aliphatic suberin region (3000-2850 cm-1)
    annotate("rect", xmin = 2825, xmax = 3000, ymin = -Inf, ymax = Inf, 
             alpha = 0.12, fill = "#5076ab") +
    # Specific aliphatic peaks
    # asymmetric CH2 stretch region (2930, 2925, 2919 cm-1)
    annotate("rect", xmin = 2923, xmax = 2928, ymin = -Inf, ymax = Inf, 
             alpha = 0.54, fill = "#5076ab") +
    # geom_vline(xintercept = 2854, linetype = "longdash", linewidth = 0.23, color = "#3D6AA9") +
    # geom_vline(xintercept = 2856, linetype = "longdash", linewidth = 0.23, color = "#3D6AA9") +
    annotate("rect", xmin = 2853, xmax = 2858, ymin = -Inf, ymax = Inf, 
             alpha = 0.54, fill = "#5076ab") +
    ...
    # Suberin esters (1735-1742 cm-1)
    annotate("rect", xmin = 1730, xmax = 1748, ymin = -Inf, ymax = Inf, 
             alpha = 0.54, fill = "#4d653a") +
    # geom_vline(xintercept = 1706, linetype = "solid", linewidth = 0.33, alpha = 0.33, color = "#4d653a") + # free acid C=O stretch Garcia et al. 2015

    # suberin fingerprint (1464-62, 1374, 1245-40, 1179-74, 723-20 cm-1)
    annotate("rect", xmin = 1454, xmax = 1472, ymin = -Inf, ymax = Inf, 
             alpha = 0.54, fill = "#5f3e38") +
    annotate("rect", xmin = 1355, xmax = 1375, ymin = -Inf, ymax = Inf, 
             alpha = 0.54, fill = "#5f3e38") +
    annotate("rect", xmin = 1233, xmax = 1253, ymin = -Inf, ymax = Inf, 
             alpha = 0.54, fill = "#5f3e38") +
  ...
  annotate("label", x = 1290, y = max_y_three, label = "suberin fingerprint", 
            size = 4.8, color = annotation_text_color, hjust = 0.5, fill = annotation_fill_color, label.size = NA)
...
ggsave_themed("combined_overlay_plot_suberin.png", combined_overlay_plot_suberin)
```
...
# Try to find the most recent processed integration data file
suber_ftir_file <- most_recent_suberin_file(outputs_folder)
