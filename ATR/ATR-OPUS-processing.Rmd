...
corr_spectra <- file.path(box_base, "Salk Institute Project/Salk Data/FTIR_Intact_decomposition_pots/ATR/ATR_OPUS_files")
# set dpt_folder path
dpt_folder <- file.path(box_base, "Salk Institute Project/Salk Data/FTIR_Intact_decomposition_pots/ATR/ATR_DPT_files")
...
for (combination in unique_combinations) {
  ...
  if (length(idx) > 1) {
    ...
    # Check if wavenumbers data is valid and finite
    wavenum_data <- spectra[[1]]$ab$wavenumbers
    if (is.null(wavenum_data) || !all(is.finite(wavenum_data)) || length(wavenum_data) == 0) {
      cat("Skipping", combination, "- invalid wavenumber data\n")
      next
    }
    
    # Get y-range from all spectra data, ensuring finite values
    all_y_data <- unlist(lapply(spectra, function(x) {
      if (!is.null(x$ab$data) && is.numeric(x$ab$data)) {
        x$ab$data[is.finite(x$ab$data)]
      } else {
        NULL
      }
    }))
    
    if (length(all_y_data) == 0) {
      cat("Skipping", combination, "- no valid absorbance data\n")
      next
    }
    
    plot(NULL, xlim = rev(range(wavenum_data, na.rm = TRUE)),
         ylim = range(all_y_data, na.rm = TRUE),
         xlab = "Wavenumber (cm⁻¹)", ylab = "Absorbance",
    ...
    for (i in seq_along(spectra)) {
      ...
      if (!is.null(ab_data) && 
          ...
          all(is.finite(ab_data$wavenumbers)) &&
          all(is.finite(ab_data$data)) &&
          length(ab_data$wavenumbers) == length(ab_data$data)) {
...
for (combo in combos) {
  ...
  if (length(idx) > 1) {
    ...
    wavenum_data <- spectra[[1]]$ab$wavenumbers
    if (is.null(wavenum_data) || !all(is.finite(wavenum_data)) || length(wavenum_data) == 0) {
      cat("Skipping", combo, "- invalid wavenumber data\n")
      next
    }
    ...
    if (length(all_y_data) == 0) {
      cat("Skipping", combo, "- no valid absorbance data\n")
      next
    }
...
These plots are of normalized absorbance values. The thickness of the opaque line represents the range of the data. Dotted lines at specific wavenumbers indicate key absorption features of suberin.
- According to Martins et al. 2014:
    > Major peaks, which can be almost exclusively assigned to suberin, are at 2921 cm-1, 2851 cm-1 and 1737 cm-1.
...
These plots are of normalized absorbance values. The thickness of the line represents the range of the data. Dotted lines at specific wavenumbers indicate key absorption features of suberin.
- According to Martins et al. 2014:
    > Major peaks, which can be almost exclusively assigned to suberin, are at 2921 cm-1, 2851 cm-1 and 1737 cm-1.
    - Martins, I., Hartmann, D.O., Alves, P.C. et al. Elucidating how the saprophytic fungus Aspergillus nidulans uses the plant polyester suberin as carbon source. BMC Genomics 15, 613 (2014). https://doi.org/10.1186/1471-2164-15-613
